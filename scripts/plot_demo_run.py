#!/usr/bin/env python3
"""Plot the 5-day hardware demo run (June 7-13 2026) for Figure 4.8 (fig:demo_run).

Three stacked panels on a shared, plain-language time axis:
  (top)    Tomato ripeness — camera colour index X (flat: RGB565 perception gap)
...
Readability choices:
  ...
  * The ~28 h earthquake-brownout outage is COMPRESSED to a single labelled
    break so the live data fills the figure instead of a large empty band.
...
CSV  = ROOT / "data/demo-2026-06-10.csv"
OUT  = ROOT / "figures/demo_run.png"
...
def main():
    ...
    ax0.set_title("Tomato ripeness (camera) — flat: never reaches harvest threshold",
    ...
    axa.set_xlabel("Date (PHT, 2026) — midnight separators; ∥ = compressed power outage",
                   fontsize=9.5)
    for ax in panels:
    ...
    print("\n--- suggested caption ---")
    print(
        f"Figure. Five-day hardware demo run (PHT). Panel 1 (ripeness): the camera "
        f"index X stays flat at {min(xv):.2f}-{max(xv):.2f} — the RGB565 capture path "
        f"compresses X and never reaches the {HARVEST_X} harvest threshold (a perception "
        f"gap; harvest is decided on the model ODE state, not this signal). Panel 2 "
        f"(chamber temperature): follows the diurnal cycle while the target setpoint sits at "
        f"the 12.5 °C floor — the undersized TEC cannot close the ~19 °C gap against 28-38 °C "
        f"tropical ambient (an actuator-sizing limit, not a policy error). Panel 3 (ESP32 "
        f"die temperature): sustained {esp_lo:.0f}-{esp_hi:.0f} °C self-heating with "
        f"continuous camera capture (the dip is the Jun-11 power-cycle), motivating the "
        f"thermal-hardening recommendation. Panel 4 "
        f"(decision): per-step controller action ({100*ac['cool']/tot:.1f}% cool / "
        f"{100*ac['heat']/tot:.1f}% heat / {100*ac['maintain']/tot:.1f}% maintain across "
        f"{tot:,} cleaned steps). The dashed line marks the Jun-11 Q1 heater-MOSFET "
        f"replacement; the break (∥) compresses a ~28 h grid power outage from a Mindanao "
        f"earthquake (elapsed model time is NVM-persisted, so the run is unaffected)."
    )
